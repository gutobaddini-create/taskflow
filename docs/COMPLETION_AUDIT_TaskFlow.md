...
The full original goal is not fully complete yet because Firebase runtime push/sync QA, physical-device QA, and production signing require external inputs that are not currently available on this machine/session.
...
- `npm run verify:firebase-real`: Firebase CLI auth, Android config, Auth e-mail/senha, Firestore rules dry-run, and Storage rules dry-run are ready.
...
| Real Firebase Firestore, Storage, and FCM product validation | Firestore/Storage dry-runs passed and rules were deployed to project `gen-lang-client-0780081219`; FCM token path is implemented but still needs runtime/device validation | Partially complete; FCM/runtime QA pending |
...
1. Runtime Firebase QA inputs: test account/device session for real sync/upload/FCM token validation.
...
1. Validate real Firebase runtime flows in the app: sign in, sign up, queued sync write, attachment upload/delete, and FCM token registration.
